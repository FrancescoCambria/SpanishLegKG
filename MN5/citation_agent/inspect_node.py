...
def inspect_node(node_id, graph_json_path, output_md=None, output_json=None):
    ...
    # Map parent document for section/article nodes via HAS_SECTION
    ...
    # Target node parent document if applicable
    ...
    for r in rels:
        if r.get("target") == target_id or str(r.get("target")) == str(target_id):
            ...
            p_info = _get_parent_doc_info(src_id)
            if p_info:
                rel_item["parent_document"] = p_info
    ...
    for r in rels:
        if r.get("source") == target_id or str(r.get("source")) == str(target_id):
            ...
            p_info = _get_parent_doc_info(tgt_id)
            if p_info:
                rel_item["parent_document"] = p_info
    ...
    if target_parent_info:
        print(f"Parent Law Document: Node {target_parent_info['parent_id']} ({target_parent_info['parent_title']})")
    ...
    for i, r in enumerate(incoming_rels, 1):
        ...
        if r.get("parent_document"):
            print(f"     Parent Law Document: Node {r['parent_document']['parent_id']} ({r['parent_document']['parent_title']})")
        if rprops:
...
def main():
    ...
    # Try parsing node_id as int if numeric
