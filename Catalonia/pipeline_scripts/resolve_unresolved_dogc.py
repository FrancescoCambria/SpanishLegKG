...
def extract_text_from_pdf_bytes(pdf_bytes):
    """
    Extracts text from PDF bytes using pdftotext or docling.
    """
    if not pdf_bytes:
        return ""
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Resolve unresolved DOGC documents using PDF text parsing and summaryDOGC matching")
    parser.add_argument("--limit", type=int, default=100, help="Maximum number of unresolved DOGC entries to process (default: 100, set 0 for all)")
    ...
    dogc_by_id, _ = load_dogc_reference_data(os.path.join(cat_root, "data"), verbose=args.verbose)
    existing_dogc_list = []
    if os.path.exists(dogc_json_path):
        with open(dogc_json_path, "r", encoding="utf-8") as f:
            existing_dogc_list = json.load(f)

    # Collect target unresolved DOGC document entries
    unresolved_targets = []
    ...
    print(f"Found {len(unresolved_targets)} total unresolved DOGC document references.")
    if args.limit > 0:
