...
def main():
    ...
    bop_path = os.path.join(data_dir, "bopg_bopt_documents.json")
    cido_map_path = os.path.join(data_dir, "cido_to_dogc_map.json")
    report_path = os.path.join(data_dir, "bopg_bopt_cido_matching_report.json")
    
    if not os.path.exists(bop_path):
        print(f"Error: {bop_path} not found. Please run download_bopg_bopt_documents.py first.")
        sys.exit(1)
    ...
    print(f"Loading BOPG & BOPT documents from {bop_path}...")
    with open(bop_path, "r", encoding="utf-8") as f:
        bop_docs = json.load(f)
    print(f"Loaded {len(bop_docs)} BOPG/BOPT document records.")
    ...
    print("\n--- Checking BOPG & BOPT Matches against CIDO Map ---")
    matched_records = []
    unmatched_records = []
    
    stats_by_bulletin = defaultdict(lambda: {"total": 0, "matched": 0, "unmatched": 0})
    stats_by_category = defaultdict(lambda: {"total": 0, "matched": 0, "unmatched": 0})
    
    for doc in bop_docs:
        bulletin = doc.get("bulletin") or "UNKNOWN"
        category = doc.get("category") or "other"
        
        stats_by_bulletin[bulletin]["total"] += 1
        ...
        if matched_cido_id:
            stats_by_bulletin[bulletin]["matched"] += 1
    ...
    report = {
        ...
        "summary": {
            "total_bop_documents_checked": total_docs,
            "total_matched_in_cido": total_matched,
            "total_unmatched_in_cido": total_unmatched,
            "match_percentage": round(match_percentage, 2)
        },
        "breakdown_by_bulletin": dict(stats_by_bulletin),
    ...
    print(f"\n--- Matching Summary ---")
    print(f"Total BOPG/BOPT Documents Checked: {total_docs}")
    print(f"Matched in CIDO Map                : {total_matched} ({match_percentage:.2f}%)")
    print(f"Unmatched                           : {total_unmatched}")
    
    print("\nBulletin Breakdown:")
    for bul, b_stats in stats_by_bulletin.items():
        pct = (b_stats["matched"] / b_stats["total"] * 100) if b_stats["total"] > 0 else 0
        print(f"  {bul:<10}: {b_stats['matched']}/{b_stats['total']} matched ({pct:.2f}%)")
